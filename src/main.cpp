...
// The rest of this code is a generic initialization and animation loop that can
// be applied to different scenes
// *************************** //
...
int main(int, char* argv[]) {
  ...
  //  The following part is simply a loop that call the function
  //  "animation_loop" (This call is different when we compile in standard mode
  //  with GLFW, than when we compile with emscripten to output the result in a
  //  webpage.)
...
void animation_loop() {
  emscripten_update_window_size(
      scene.window.width,
      scene.window.height);  // update window size in case of use of emscripten
                             // (not used by default)
...
void initialize_default_shaders() {
  ...
  mesh_drawable::default_shader.load(
      default_path_shaders + "mesh/mesh.vert.glsl",
      default_path_shaders + "mesh/mesh.frag.glsl");
  triangles_drawable::default_shader.load(
      default_path_shaders + "mesh/mesh.vert.glsl",
      default_path_shaders + "mesh/mesh.frag.glsl");

  // Set default white texture
  image_structure const white_image =
      image_structure{1, 1, image_color_type::rgba, {255, 255, 255, 255}};
  ...
  curve_drawable::default_shader.load(
      default_path_shaders + "single_color/single_color.vert.glsl",
...
window_structure standard_window_initialization() {
  ...
  if (project::initial_window_size_width < 1)
    window_width =
        project::initial_window_size_width * scene.window.monitor_width();
  if (project::initial_window_size_height < 1)
    window_height =
        project::initial_window_size_height * scene.window.monitor_height();

  // Create the window using GLFW
  window_structure window;
  window.create_window(window_width, window_height, "CGP Display",
                       CGP_OPENGL_VERSION_MAJOR, CGP_OPENGL_VERSION_MINOR);

  // Display information
  // ***************************************************** //

  // Display window size
  std::cout << "\nWindow (" << window.width << "px x " << window.height
            << "px) created" << std::endl;
  std::cout << "Monitor: " << glfwGetMonitorName(window.monitor)
            << " - Resolution (" << window.screen_resolution_width << "x"
            << window.screen_resolution_height << ")\n"
            << std::endl;
  ...
  glfwSetMouseButtonCallback(
      window.glfw_window,
      mouse_click_callback);  // Event called when a button of the mouse is
                              // clicked/released
  glfwSetCursorPosCallback(
      window.glfw_window,
      mouse_move_callback);  // Event called when the mouse is moved
  glfwSetWindowSizeCallback(
      window.glfw_window,
      window_size_callback);  // Event called when the window is rescaled
  glfwSetKeyCallback(window.glfw_window,
                     keyboard_callback);  // Event called when a keyboard touch
                                          // is pressed/released
  glfwSetScrollCallback(
      window.glfw_window,
      mouse_scroll_callback);  // Event called when scrolling the mouse
...
void mouse_move_callback(GLFWwindow* /*window*/, double xpos, double ypos) {
  vec2 const pos_relative =
      scene.window.convert_pixel_to_relative_coordinates({xpos, ypos});
...
void display_gui_default() {
  ...
  if (ImGui::CollapsingHeader("Window")) {
    ...
    //  This limits the risk of having different behaviors when you use
    //  different machine.
...
#endif
    // vsync is the default synchronization of frame refresh with the screen
    // frequency
    //   vsync may or may not be enforced by your GPU driver and OS (on top of
    //   the GLFW request). de-activating vsync may generate arbitrary large FPS
    //   depending on your GPU and scene.
    if (ImGui::Checkbox("vsync (screen sync)", &project::vsync)) {
      project::vsync == true ? glfwSwapInterval(1) : glfwSwapInterval(0);
    }

    std::string window_size = "Window " + str(scene.window.width) + "px x " +
                              str(scene.window.height) + "px";
